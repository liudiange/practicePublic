...
- (void)showGift:(LDGGiftModel *)giftModel{
    ...
    for (NSInteger index = 0; index < self.subviews.count; index ++) {
        ...
        if ([channelView.model isEqualsModel:giftModel] && (channelView.currentState != CurrentStateWillEndAnimation)){// 同一个人送的同一个礼物
            // 添加缓存
            [channelView addCacheNumber];
            indexNumber = index;
            break;
        }else if (channelView.currentState == CurrentStateIdle) { // 闲置的
...
- (void)channelIsFinishAnimation:(LDGChannelView *)channelView{
    if (self.cacheModelArray.count) {
        LDGGiftModel *giftModel = [self.cacheModelArray firstObject];
        [channelView showChannelModel:giftModel];
        [self.cacheModelArray removeObject:giftModel];
    }
}
