        ...
        [manager POSTWithRequestUrl:@"http://www.baidu.com" param:nil complete:^(DGBaseHttpModel *baseModel) {
            if (!baseModel.serverError) {
                for (int i = 0; i < 8; i++) {
                    ...
                    [self.listHeaderViewModel.headerDataArray addObject:model];
                }
                [self.refreashUI sendNext:nil];
                [self.listHeaderViewModel.refreshUI sendNext:nil];
            }else{
                [self.refreashUI sendNext:nil];
                [self.listHeaderViewModel.refreshUI sendNext:nil];
            }
...
- (DGListHeaderViewModel *)listHeaderViewModel {
    if (!_listHeaderViewModel) {
        _listHeaderViewModel = [[DGListHeaderViewModel alloc] init];
    }
